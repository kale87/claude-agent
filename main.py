...
with get_db() as conn:
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS telemetry (
        ...
        task_id TEXT NOT NULL, project_name TEXT NOT NULL DEFAULT '',
        interaction_count INTEGER NOT NULL DEFAULT 0, status TEXT NOT NULL DEFAULT 'running',
...
def save_session(sid: str, title: str, messages: list):
    now = int(datetime.now().timestamp() * 1000)
    with get_db() as conn:
        conn.execute("""INSERT INTO sessions (id,title,messages,last_accessed) VALUES (?,?,?,?)
...
To commit code directly to GitHub, wrap your final file content EXACTLY like this:
...
Use this whenever you write code that should be saved. Do NOT tell the user to commit manually.""",
    },
    "analyst": {
        "name": "Analyst", "emoji": "\U0001f50d", "color": "#f59e0b",
        "system": "You are the Analyst agent. Research topics, analyze information, produce clear structured summaries.",
    },
}
...
def parse_github_intent(message: str):
    ...
    if re.search(r'merge\s+(?:pull.?request|pr)', msg):
        ...
        return None, None
    if not repo and re.search(r'(list|show|get|what).{0,30}repos?', msg): return 'list_repos', {}
    if not repo and re.search(r'\brepos?\b', msg): return 'list_repos', {}
    ...
    if repo and file_path: return 'read_file', {'owner': GITHUB_USER, 'repo': repo, 'path': file_path, 'branch': branch}
    if repo and re.search(r'branch(es)?', msg): return 'list_branches', {'owner': GITHUB_USER, 'repo': repo}
    if repo and re.search(r'(pull.?request|\bpr\b)', msg): return 'list_prs', {'owner': GITHUB_USER, 'repo': repo, 'state': 'open'}
    if repo: return 'list_files', {'owner': GITHUB_USER, 'repo': repo, 'path': '', 'branch': ''}
    return None, None
...
async def _run_gh_intent(intent: str, p: dict) -> str:
    if intent == 'list_repos':
        ...
        for pr in data: lines.append(f"- #{pr['number']} **{pr['title']}** (`{pr['head']['ref']}` \u2192 `{pr['base']['ref']}`)")
        return "\n".join(lines)
        ...
        result = await gh("POST", f"/repos/{p['owner']}/{p['repo']}/merges",
                          {"base": p['base'], "head": p['head'], "commit_message": f"Merge {p['head']} into {p['base']}"})
        ...
        result = await gh("POST", f"/repos/{p['owner']}/{p['repo']}/pulls",
                          {"title": p['title'], "head": p['head'], "base": p.get('base','main'), "body": p.get('body','')})
        ...
        body: dict = {"message": p.get('message', f"Update {p['path']}"),
                      "content": base64.b64encode(p['content'].encode()).decode(), "branch": branch}
        try:
...
async def auto_commit_from_response(response: str, task_id: str) -> list[str]:
    """
    Parse <commit> tags from model response and commit each to GitHub.
    Returns list of commit result messages.
    """
    pattern = re.compile(
    ...
    for m in pattern.finditer(response):
        repo, path, message, branch, content = m.group(1), m.group(2), m.group(3), m.group(4) or 'main', m.group(5).strip()
        params = {'owner': GITHUB_USER, 'repo': repo, 'path': path, 'content': content, 'message': message, 'branch': branch}
        try:
...
async def chat(request: Request):
    ...
    async def generate():
        ...
        for agent_key, task in delegations:
            ...
            # AUTO-COMMIT: parse <commit> tags from coder response
            if agent_key == "coder":
...
\U0001f680 Kal-AI running at http://localhost:{PORT}
   Model:  {OLLAMA_MODEL} | Ollama: {OLLAMA_HOST}
